...
export default function SubeBaskanlariAnmeldungPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch('/api/registrations/sube-baskanlari', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          firstName: firstName.trim(),
          lastName: lastName.trim(),
          sube: sube.trim() || undefined,
      ...
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50 p-4">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl text-center">
          ...
          <p className="text-gray-600">
            Ihre Anmeldung für Şube Başkanları wurde erfolgreich übermittelt. Wir freuen uns auf Ihre Teilnahme.
          </p>
          <button
            type="button"
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50 p-4">
      <div className="anmeldung-form w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl">
        <h1 className="mb-2 text-center text-3xl font-bold text-gray-900">Şube Başkanları</h1>
        <p className="mb-6 text-center text-gray-600">
          Registrierung – Teilnahme am Event bekunden
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="sube" className="mb-1 block text-sm font-medium text-gray-700">
              Şube
            </label>
            <input
              id="sube"
              ...
              className="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="Aus welchem Bezirk / welche Şube? (z. B. Berlin-Mitte, Kreuzberg)"
            />
          </div>
