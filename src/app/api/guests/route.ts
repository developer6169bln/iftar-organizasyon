...
// DELETE Column: Entfernt eine Spalte aus additionalData aller Gäste
export async function DELETE_COLUMN(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url)
    const eventId = searchParams.get('eventId')
    const columnName = searchParams.get('columnName')

    if (!eventId || !columnName) {
      return NextResponse.json(
        { error: 'eventId und columnName sind erforderlich' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Fehler beim Löschen der Spalte', details: errorMessage },
      { status: 500 }
    )
  }
}
